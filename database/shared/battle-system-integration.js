...
class EnhancedBattleSystemManager {
    ...
    calculateRealTeamRating(team) {
        ...
        breakdown.chemistry = this.calculateAdvancedChemistry(breakdown.starters, breakdown.bench);
        
        // Calculate bonuses based on real player tiers
        breakdown.bonuses = this.calculateAdvancedBonuses(breakdown.starters, breakdown.bench);
        ...
        return {
            ...
            averageStarter: Math.round(breakdown.starterTotal / Math.max(1, breakdown.starters.length))
        };
    }
    ...
    calculateAdvancedChemistry(starters, bench) {
        ...
        const youngCore = allPlayers.filter(p => p.overall >= 75 && p.overall <= 85);
    ...
    conductPositionBattles(team1Rating, team2Rating) {
        ...
        const team1Wins = Object.values(results).filter(r => r.winner === 'team1').length;
        const team2Wins = Object.values(results).filter(r => r.winner === 'team2').length;
    ...
    generateBattleSummary(battleResult) {
        ...
        if (battleType === 'Nail-biter') {
            summary += "This was an incredibly close matchup that could have gone either way!";
            ...
            summary += "This was a dominant performance showing superior team construction!";
        }
    ...
    getPlayerInfo(playerName) {
        ...
        return this.playerRatingLookup.getPlayerRating(playerName) || 
               { name: playerName, overall: 75, tier: 'Unknown' };
    }
}
